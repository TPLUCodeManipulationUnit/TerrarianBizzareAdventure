...
namespace TerrarianBizzareAdventure
{
    public class OnEdits
    {
        ...
        {
            bool ownerIsTimeStopper = TimeStopManagement.TimeStopper != null && TimeStopManagement.TimeStopper.player.whoAmI == self.owner;
            bool isNativelyImmune = self.modProjectile != null && self.modProjectile is IProjectileHasImmunityToTimeStop stop && stop.IsNativelyImmuneToTimeStop();

            if (!TimeStopped
                || TimeStopManagement.TimeStopImmuneProjectiles.Contains(self.type)
                || ownerIsTimeStopper
                || isNativelyImmune)
                orig.Invoke(self, i);
        }
        ...
        private void NPC_VanillaHitEffect(On.Terraria.NPC.orig_VanillaHitEffect orig, NPC self, int hitDirection, double dmg)
        {
            ...
            {
                if (TimeStopManagement.npcStates.ContainsKey(self))
                {
                    TimeStopManagement.npcStates[self].AccumulatedDamage += (int)dmg;
                    self.life += (int)dmg;
                }
            }
        }
        ...
        public bool TimeStopped => TimeStopManagement.TimeStopped;
    }
}
